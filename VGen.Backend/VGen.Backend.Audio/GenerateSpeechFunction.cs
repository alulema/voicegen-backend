...
using VGen.Backend.Audio.Model;
using User = VGen.Backend.Audio.Model.User;

namespace VGen.Backend.Audio
{
    public class GenerateSpeechFunction
    {
        private readonly ILogger<GenerateSpeechFunction> _logger;
        ...
        public GenerateSpeechFunction(ILogger<GenerateSpeechFunction> logger)
        {
            _logger = logger;
            CosmosClient localCosmosClient = new(EndpointUrl, PrimaryKey, new CosmosClientOptions { ApplicationName = "VoiceGeneratorApp"});
            Database localDatabase = localCosmosClient.CreateDatabaseIfNotExistsAsync(DatabaseId).GetAwaiter().GetResult();
        ...
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Function, "post", Route = "generate-speedch")] HttpRequest req)
        {
            string email;
